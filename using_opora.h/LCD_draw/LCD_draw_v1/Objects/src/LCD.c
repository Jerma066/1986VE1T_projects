...
{
//порт А - шина данных
	RST_CLK->PER_CLOCK |= 0x115<<21;	//Тактирование портов A, C, E

	PORTA->FUNC &= 0xFFFF0000;	//PORTA[7:0] - основная функция (шина данных)
	PORTA->FUNC |= 0x00005555;	//PORTA[7:0] - основная функция (шина данных)
	PORTA->ANALOG |= 0x00FF;	//PORTA[7:0] - digital
	PORTA->PWR |= 0x0000FFFF;

	PORTC->FUNC &= 0xFFFFFFCC;	//PORTC 0 - основная функция
	PORTC->FUNC |= 0x00000021;	//PORTC 0 - основная функция
	PORTC->ANALOG |= 0x0005;
	PORTC->PWR |= 0x00000031;

	PORTE->FUNC &= 0xC0FFFFFF;	//PORTE 12 - основная функция (шина адреса)
	PORTE->FUNC |= 0x2A000000;	//PORTE 12 - основная функция (шина адреса)
	PORTE->ANALOG |= 0x7000;	
	PORTE->PWR |= 0x3F000000;

	PORTF->FUNC = 0;	//PORTE 12, 13, 14 - основная функция (шина адреса)
	PORTF->RXTX = 0;
	PORTF->OE |= 0x0001;
	PORTF->ANALOG = 0x0001;	
...
void SetPage(uint32_t Page, _crystal Crystal)
{
	if(Crystal == First)
	{	
		*((uint32_t*)LCD_CMD1) = (0xB8|(Page&0x07));
	}
	else
	{
		*((uint32_t*)LCD_CMD2) = (0xB8|(Page&0x07));
	}
	Pause8us();
}
...
void SetAddress(uint32_t Address)
{
	if(Address < 64)
	{
		*((uint32_t*)LCD_CMD1) = (0x40|(Address&0x3F));
	}
	else
	{
		*((uint32_t*)LCD_CMD2) = (0x40|(Address&0x3F));	
	}
	Pause8us();
}
...
uint8_t ReadStatus(_crystal Crystal)
{
	...
	if(Crystal == First)
	{
		Temp = *((uint32_t*)LCD_CMD1);
	}
	else
	{
		Temp = *((uint32_t*)LCD_CMD2);
	}
	Pause8us();
	return Temp;
}
...
void WriteByte(uint8_t Byte, uint8_t Address)
{
	...
	{
		SetAddress(Address);
		*((uint32_t*)LCD_DATA1) = Byte;
	}
	else
	{
		SetAddress(Address);
		*((uint32_t*)LCD_DATA2) = Byte;
	}
	Pause8us();
}
...
void ClearPageAtTo(uint32_t At, uint32_t To, uint32_t Page)
{
	if(At>=To) return;
	...
	else
	{
		while(At++ <To)
		{
			if(At<64)	*((uint32_t*)LCD_DATA1) = 0x00;
			else		*((uint32_t*)LCD_DATA2) = 0x00;
			Pause8us();
		}
	}
}
