...
{
//порт А - шина данных
	MDR_RST_CLK->PER_CLOCK |= 0x15<<21;	//Тактирование портов A, C, E
...
//для получения основной функции порта xn в поле FUNС структуры PORTx записывае 1 на позицию 2n и 0 на позицию (2n+1)	
	...
	MDR_PORTA->PWR |= 0x0000FFFF;		//используем максимально быстрый фронт для шины данных

	MDR_PORTC->FUNC |= 0x00000021;	//PORTC 0 - основная функция PORTC 2 - альтернативная 
	MDR_PORTC->ANALOG |= 0x0005; 		//PORTC0 и PORTC1 - digital
	MDR_PORTC->PWR |= 0x00000051;		//используем максимально быстрый фронт

	MDR_PORTE->FUNC |= 0x2A000000;	//PORTE 12, 13, 14 - альтернативная функция (A26, A27, A28)
	MDR_PORTE->ANALOG |= 0x7000;		//digital 
	MDR_PORTE->PWR |= 0x3F000000; 	//используем максимально быстрый фронт
	
	MDR_PORTF->FUNC = 0;						//PORTF0 - функция порт
	MDR_PORTF->RXTX = 0;						//Режим работы вывода порта Данные для выдачи на выводы порта и для чтения. 
	MDR_PORTF->OE |= 0x0001;				//Направление порта на выход
	MDR_PORTF->ANALOG = 0x0001;		  //digital
	MDR_PORTF->PWR |= 0x00000003;		//используем максимально быстрый фронт
...
//Функция сброса LCD-модуля (хочу его удалить! Зачем он нужен?)
void ResetLCD()
{
	MDR_PORTF->CLRTX = 1;
	Pause8us();
